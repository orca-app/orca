...
void test_resolve(oc_test_info* info, oc_arena* arena)
{
    ...
    oc_test(info, "path with valid ../.. outside root")
    {
        oc_io_resolve_result r = oc_io_resolve(arena, dirSlot->fd, OC_STR8("../../files/data/directory/test.txt"), 0);
        if(r.error || oc_str8_cmp(r.path, OC_STR8("../../files/data/directory/test.txt")))
        {
            oc_test_fail(info, "Bad path resolution.");
        }
        oc_fd_close(r.fd);
