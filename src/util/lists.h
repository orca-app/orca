...
#define oc_list_entry(elt, type, linksName) \
    oc_container_of(elt, type, linksName)

#define oc_list_next_entry(elt, type, linksName) \
    ((elt->linksName.next != 0) ? oc_list_entry(elt->linksName.next, type, linksName) : 0)

#define oc_list_prev_entry(elt, type, linksName) \
    ((elt->linksName.prev != 0) ? oc_list_entry(elt->linksName.prev, type, linksName) : 0)

#define oc_list_checked_entry(elt, type, linksName) \
    (((elt) != 0) ? oc_list_entry(elt, type, linksName) : 0)

#define oc_list_first_entry(list, type, linksName) \
    (oc_list_checked_entry(oc_list_begin(list), type, linksName))

#define oc_list_last_entry(list, type, linksName) \
    (oc_list_checked_entry(oc_list_last(list), type, linksName))

#define oc_list_for(list, elt, type, linksName)                                  \
    for(type* elt = oc_list_checked_entry(oc_list_begin(list), type, linksName); \
        elt != 0;                                                                \
        elt = oc_list_checked_entry(elt->linksName.next, type, linksName))

#define oc_list_for_reverse(list, elt, type, linksName)                         \
    for(type* elt = oc_list_checked_entry(oc_list_last(list), type, linksName); \
        elt != 0;                                                               \
        elt = oc_list_checked_entry(elt->linksName.prev, type, linksName))

#define oc_list_for_indexed(list, it, type, linksName)                                      \
    for(                                                                                    \
        struct { size_t index; type* elt; } it = { 0, oc_list_checked_entry(oc_list_begin(list), type, linksName) }; \
        it.elt != 0;                                                                        \
        it.elt = oc_list_checked_entry(it.elt->linksName.next, type, linksName), it.index++)

#define oc_list_for_safe(list, elt, type, linksName)                                          \
    for(type* elt = oc_list_checked_entry(oc_list_begin(list), type, linksName),              \
              *__tmp = elt ? oc_list_checked_entry(elt->linksName.next, type, linksName) : 0; \
        elt != 0;                                                                             \
        elt = __tmp,                                                                          \
              __tmp = elt ? oc_list_checked_entry(elt->linksName.next, type, linksName) : 0)

#define oc_list_pop_front_entry(list, type, linksName) (oc_list_empty(*list) ? 0 : oc_list_entry(oc_list_pop_front(list), type, linksName))
#define oc_list_pop_back_entry(list, type, linksName) (oc_list_empty(*list) ? 0 : oc_list_entry(oc_list_pop_back(list), type, linksName))
