...
{
	oc_ui_style style = {.size.width = {OC_UI_SIZE_PARENT, 1, 0},
	                  .size.height = {OC_UI_SIZE_CHILDREN},
...
typedef enum {
	...
	OC_UI_EDIT_COPY,
	OC_UI_EDIT_PASTE,
	OC_UI_EDIT_SELECT_ALL } oc_ui_edit_op;

typedef enum {
	OC_UI_EDIT_MOVE_NONE = 0,
	OC_UI_EDIT_MOVE_ONE,
	OC_UI_EDIT_MOVE_WORD,
	OC_UI_EDIT_MOVE_LINE } oc_ui_edit_move;
	...
	switch(operation)
	{
		case OC_UI_EDIT_MOVE:
		{
			...
			u32 cursor = direction < 0 ?
				         oc_min(ui->editCursor, ui->editMark) :
				         oc_max(ui->editCursor, ui->editMark);
