...
void* ORCA_IMPORT(oc_hostcall_mem_grow)(u64 size);

#define MORECORE oc_hostcall_mem_grow
...
#define REQUEST_OUT_OF_RANGE(req) \
    ((CHUNK_SIZE_T)(req) >= (CHUNK_SIZE_T)(INTERNAL_SIZE_T)(-2 * MINSIZE))
...
static void do_check_malloc_state(void)
{
    ...
    for(i = 1; i < NBINS; ++i)
    {
        ...
        for(p = last(b); p != b; p = p->bk)
        {
            ...
            for(q = next_chunk(p);
                (q != av->top && inuse(q) && (CHUNK_SIZE_T)(chunksize(q)) >= MINSIZE);
                q = next_chunk(q))
...
#endif
{
  ...
  */
    ...
    assert((old_top == initial_top(av) && old_size == 0) || ((CHUNK_SIZE_T)(old_size) >= MINSIZE && prev_inuse(old_top)));
...
static void* sbrk(long size)
{
    ...
    /* Assert invariants */
    assert(g_last);
    assert((char*)g_last->top_reserved - g_last->reserve_size <= (char*)g_last->top_allocated && g_last->top_allocated <= g_last->top_committed);
    assert((char*)g_last->top_reserved - g_last->reserve_size <= (char*)g_last->top_committed && g_last->top_committed <= g_last->top_reserved && (unsigned)g_last->top_committed % g_pagesize == 0);
                ...
                }
                {
                    ...
                    do
                    {
                        ...
                        while(VirtualQuery(memory_info.BaseAddress, &memory_info, sizeof(memory_info)))
                        {
                            ...
                            if(memory_info.State == MEM_FREE && (unsigned)memory_info.BaseAddress % g_regionsize == 0 && memory_info.RegionSize >= (unsigned)reserve_size)
                            {
                                found = TRUE;
                                break;
                            }
    ...
    else if(size < 0)
    {
        ...
        /* Check for underflow */
        if((char*)g_last->top_reserved - g_last->reserve_size > (char*)g_last->top_allocated || g_last->top_allocated > g_last->top_committed)
        {
            /* Adjust regions allocate top */
