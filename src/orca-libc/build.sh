...
cfiles=(src/{complex,ctype,errno,fenv,math,prng,string,stdlib,stdio,internal,malloc,multibyte,exit}/*.c)
...
includes=(-isystem ./include -Iarch -Isrc/internal -I..)
...
/usr/local/opt/llvm/bin/clang -O2 -DNDEBUG --target=wasm32 -D__ORCA__ --std=c11 --no-standard-libraries -fno-trapping-math -mbulk-memory -DBULK_MEMORY_THRESHOLD=32 -mthread-model single -MD -MP -Wl,--relocatable $warnings $includes -o lib/libc.o $cfiles
