...
cfiles=(src/{complex,ctype,errno,fenv,math,prng,string,stdlib,internal}/*.c)
...
/usr/local/opt/llvm/bin/clang -O2 -DNDEBUG --target=wasm32 --no-standard-libraries -fno-trapping-math -mbulk-memory -DBULK_MEMORY_THRESHOLD=32 -mthread-model single -MD -MP -Wl,--no-entry $warnings $includes -o stdlib.o $cfiles
