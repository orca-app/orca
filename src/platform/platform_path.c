...
oc_str8_list oc_path_split(oc_arena* arena, oc_str8 path)
{
    ...
    if(path.len > 2 && path.ptr[0] > 'A' && path.ptr[0] < 'Z' && path.ptr[1] == ':' && path.ptr[2] == '/')
    {
        //NOTE: on windows, for path beginning with a drive letter, add the first slash
        // after the colon
        oc_list_pop_front(&res.list);
