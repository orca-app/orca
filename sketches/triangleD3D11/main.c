...
#include "orca.h"
#include "graphics/d3d11_surface.h"
...
static void FatalError(const char* message)
{
    ...
    #error "D3D11 surface is not implemented on macOS\n"
#endif
...
int main()
{
    ...
    //NOTE: create surface
    oc_surface surface = oc_surface_create_for_window(window, OC_D3D11);

    if(oc_surface_is_nil(surface))
    {
        OC_ABORT("Couldn't create D3D11 surface\n");
    }

    //NOTE: init shader and pipeline state
    oc_surface_select(surface);

    ID3D11Device* device = oc_d3d11_surface_device(surface);
    ID3D11DeviceContext* context = oc_d3d11_surface_context(surface);
    HRESULT hr = 0;
...
*/
    ...
    while(!oc_should_quit())
    {
        ...
        alpha += 2 * M_PI / 120;
        alpha = fmodf(alpha, 2.0f * (float)M_PI);

        oc_vec2 frameSize = oc_surface_get_size(surface);
        oc_vec2 scaling = oc_surface_contents_scaling(surface);
        frameSize.x *= scaling.x;
        frameSize.y *= scaling.y;
        f32 aspect = frameSize.y / frameSize.x;

        //NOTE: get render target view
        ID3D11RenderTargetView* rtView = oc_d3d11_surface_get_render_target_view(surface);
        ...
        {
            float matrix[16] = {
                cosf(alpha) * aspect, -sinf(alpha), 0, 0, //
